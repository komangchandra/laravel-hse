...
class UpdatePartnerRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'short_name' => ['required', 'string', 'max:50', Rule::unique('partners')->ignore($partner)],
            'email' => ['required', 'email', 'max:255', Rule::unique('partners')->ignore($partner)],
            'organization_kind' => ['required', Rule::in([Partner::KIND_OWNER, Partner::KIND_PARTNER])],
            'owner_id' => ['nullable', 'required_if:organization_kind,partner', 'prohibited_if:organization_kind,owner', 'integer', Rule::notIn([$partner->id]), 'exists:partners,id'],
    ...
    public function after(): array
    {
        return [function (Validator $validator) {
            ...
            if ($partner->organization_kind !== $this->input('organization_kind')
                && ($partner->childPartners()->exists() || $partner->hasOperationalData())) {
                $validator->errors()->add('organization_kind', 'Jenis organisasi tidak dapat diubah karena organisasi sudah memiliki relasi atau data operasional.');
            }
        }];
    }
}
